// Left seat popup for controlling driver seat settings in the Tesla interface
import QtQuick 2.15
...
Popup {
    ...
    // Main layout for left seat controls
    ColumnLayout {
        anchors.fill: parent
        spacing: 15

        // Title for the left seat popup
        Text {
            Layout.alignment: Qt.AlignHCenter
            Layout.topMargin: 20
            text: "Left Seat Settings"
            font.family: "Inter"
            font.pixelSize: 24
        ...
        // Status text for seat heating
        Text {
            id: seatHeatingStatusText
            Layout.alignment: Qt.AlignHCenter
        ...
        Button {
            text: seatHeatingStatusText.isHeatingOn ? "Turn Off" : "Turn On"
            width: 150
            height: 50
            Layout.alignment: Qt.AlignHCenter
            onClicked: {
                seatHeatingStatusText.isHeatingOn = !seatHeatingStatusText.isHeatingOn
                console.log("Left seat heating:", seatHeatingStatusText.isHeatingOn ? "ON" : "OFF")
                logger.logMessage("Left seat heating: " + (seatHeatingStatusText.isHeatingOn ? "ON" : "OFF"))
            }
            background: Rectangle {
                color: seatHeatingStatusText.isHeatingOn ? "#f44336" : "#4caf50"
                radius: 25
            }
            contentItem: Text {
                text: parent.text
