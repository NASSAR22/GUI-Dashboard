...
Page {
    ...
    property int animationDuration: 5000 // 2 seconds per movement
    ...
    PropertyAnimation {
        ...
        onFinished: {
            if (coordinatesList.length > 1 && currentCoordinateIndex < coordinatesList.length - 1) {
                ...
                // Removed the automatic restart when reaching the end
            }
        }
    }
    ...
    // Function to move marker to next coordinate
    function moveToNextCoordinate() {
        if (currentCoordinateIndex < coordinatesList.length) {
            // Calculate duration based on distance for more natural movement
            var distance = referenceMarker.coordinate.distanceTo(coordinatesList[currentCoordinateIndex])
            animationDuration = Math.max(3000, Math.min(10000, distance * 20)) // Slower movement
    ...
    // Function to start marker movement animation
    function startMarkerMovement() {
        ...
        if (coordinatesList.length === 1) {
            // Scenario 1: Single coordinate - just position the marker
        ...
        // Scenario 2: Multiple coordinates - start animation
        isMoving = true
        currentCoordinateIndex = 0
        referenceMarker.coordinate = coordinatesList[0]
        map.center = coordinatesList[0]
    ...
    // Function to stop marker movement
    ...
    // Function to parse coordinates from text content
    function parseCoordinates(text) {
        ...
        if (validCoordinates.length > 0) {
            ...
            // Don't auto-start - let user click play button
        } else {
    ...
    function searchLocation(query) {
        ...
        for (var i = 0; i < locations.length; i++) {
            if (locations[i].nameAr.toLowerCase().includes(cleanedQuery.toLowerCase()) ||
                ...
                updateRoute(referenceCoordinate, locations[i].coordinate)
                calculateDistance()
    ...
    GeocodeModel {
        ...
        onStatusChanged: {
            ...
            if (status === GeocodeModel.Ready && count > 0) {
                ...
                updateRoute(referenceCoordinate, location.coordinate)
                calculateDistance()
                errorMessage.visible = false
    ...
    Component.onCompleted: {
        // Load default coordinates but don't auto-start
        //     loadDefaultRoute()
        // }

        // function loadDefaultRoute() {
        //     coordinatesList = [
        //                 QtPositioning.coordinate(30.4202, 31.5622), // Belbeis
        //                 QtPositioning.coordinate(30.4225, 31.5608),
        //                 // ... [all other coordinates from previous example] ...
        //                 QtPositioning.coordinate(30.5871, 31.5020)  // Zagazig University
        //             ]

        //     referenceCoordinate = coordinatesList[0]
        //     showError(isArabic ? "تم تحميل مسار من بلبيس إلى جامعة الزقازيق" :
        //                          "Loaded route from Belbeis to Zagazig University")
        //     // Don't auto-start - let user click play button
        // }
    }
}
